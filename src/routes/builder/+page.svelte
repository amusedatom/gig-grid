...
<div class="flex min-h-dvh flex-col items-center justify-center bg-gray-950 p-4 text-white">
	<div class="flex w-full max-w-md flex-col gap-6">
		...
		<div
			class="flex flex-col gap-4 rounded-xl border border-gray-800 bg-gray-900/50 p-6 shadow-2xl backdrop-blur-sm"
		>
			<!-- Grid Settings -->
			<div class="grid grid-cols-2 gap-4">
				<div class="flex flex-col gap-2">
					<label for="size-input" class="text-xs font-bold text-gray-400"
						>GRID SIZE: {gridSize}x{gridSize}</label
					>
					<input
						id="size-input"
						type="range"
				...
				<div class="flex flex-col gap-2">
					<label for="valid-input" class="text-xs font-bold text-gray-400"
						>VALID SQUARES: {validCount}</label
					>
					<input
						id="valid-input"
						type="range"
			...
			<div class="flex flex-col gap-2">
				<label for="song-input" class="text-sm font-bold text-gray-300"
					>ADD SONGS ({songs.length})</label
				>
			...
			<div
				class="flex max-h-64 flex-col gap-2 overflow-y-auto rounded-lg border border-gray-800 bg-gray-950/50 p-2"
			>
				{#if songs.length === 0}
