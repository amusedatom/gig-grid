// ========================================
// SONG DATABASE
// Pre-cached songs with short IDs for URL compression
// ========================================

/**
 * Master song database
 * Format: { id: { name: "Song Name", artist: "Artist Name" } }
 */
const SONG_DATABASE = {
    // Drum & Bass Classics
 ...
 * @returns {Array} Array of { id, name, artist, display }
 */
 ...
 * @param {Array<string>} ids - Array of song IDs
 * @returns {Array} Array of song objects
 */
function getSongsByIds(ids) {
    return ids.map(id => getSongById(id)).filter(song => song !== null);
}
...
/**
 * Get songs by genre prefix
 * @param {string} prefix - Genre prefix (e.g., 'dnb', 'hs', 'tc')
 * @returns {Array} Array of songs matching the genre
 */
function getSongsByGenre(prefix) {
    return Object.entries(SONG_DATABASE)
...
/**
 * Get genre name from prefix
 * @param {string} prefix - Genre prefix
 * @returns {string} Genre name
 */
function getGenreName(prefix) {
    const genreMap = {
        'dnb': 'Drum & Bass',
        'hs': 'House',
        'tc': 'Techno',
    ...
    return genreMap[prefix] || 'Unknown';
}

/**
 * Get all available genres
 * @returns {Array} Array of { prefix, name, count }
 */
function getAllGenres() {
    const genres = {};

    Object.keys(SONG_DATABASE).forEach(id => {
        const prefix = id.match(/^[a-z]+/)?.[0];
        if (prefix) {
            genres[prefix] = (genres[prefix] || 0) + 1;
        }
    });

    return Object.entries(genres).map(([prefix, count]) => ({
        prefix,
        name: getGenreName(prefix),
        count
    }));
}
